...
> [!IMPORTANT]
> 本文档是历史方案，包含多账本、CSV 导出等已取消范围，不再作为实施依据。当前基线请参阅[微信共享记账小程序完整开发设计方案 V2.0](03-微信共享记账小程序完整开发设计方案.md)。
...
本项目采用京东云 `117.72.101.42` 完成生产部署：Nginx 对外提供 HTTPS，Spring Boot 以 JAR + systemd 运行，MySQL 8 以系统服务运行。服务器只开放 80、443 和受限来源的实际 SSH 端口；Spring Boot 的 7631 与 MySQL 的 3306 仅监听本机。
...
flowchart LR
    ...
    NG -->|"127.0.0.1:7631"| APP["Spring Boot JAR\nsystemd 托管"]
    APP -->|"127.0.0.1:3306"| DB["MySQL 8"]
...
| 数据库 | MySQL 8（实际小版本以 `SELECT VERSION()` 为准）、InnoDB、utf8mb4 |
| 网关 | Nginx 系统包 |
...
- 安全组：80/443 对公网，实际 SSH 端口只允许固定运维 IP；不放行 7631、3306。
...
MySQL 8 已安装在 `117.72.101.42`。不要为了追求不同的 8.x 小版本而混用 Ubuntu 自带 MySQL 与 Oracle 仓库；先执行 `SELECT VERSION()` 确认实际版本。数据库服务验收命令：
...
curl -fsS http://127.0.0.1:7631/actuator/health
```
...
- 生产只公开业务 API；Actuator、Swagger、7631、3306 均不公开。
...
11. Nginx 只允许 HTTPS 业务 API，7631/3306 外网不可达。
...
- [ ] 生产服务器只开放 80/443 和受限来源的实际 SSH 端口。
- [ ] Java 监听 `127.0.0.1:7631`；MySQL 监听 `127.0.0.1:3306`。
