...
目标 API 主机暂定为 `www.cr-chenny.com`，服务器为 `117.72.101.42`。备案、DNS 和受信任证书通过前，两个地址只作为计划参数，不启用小程序请求。
...
- Nginx 安装脚本固定使用 `/opt/nginx` 作为项目管理根目录；Ubuntu 官方包提供 `/usr/sbin/nginx`，备案/证书前运行回环 `8081` 模式，证书就绪后切换 443 HTTPS 模式。
- 备案通过后统一把 API 主机替换为 `www.cr-chenny.com`；若改用 `api.cr-chenny.com`，只需同步替换 Nginx、证书、微信合法域名和小程序地址。
...
完成 C 阶段后，日常发布使用 `deploy/scripts/deploy.ps1`：默认 dry-run；仅在服务器预检、备份 option 文件、systemd 和 HTTPS 已通过后，使用 `-Apply -ConfirmText DEPLOY` 执行一次受控发布。
