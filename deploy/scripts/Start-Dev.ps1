...
$messages = @{
    ...
    StartupDependencyMissing = 'The configured Java, Windows OpenSSH, PowerShell, or backend script is missing.'
    ...
    SourceMigrationInventoryRejected = 'Migration source files do not match the approved inventory. No rebuild was attempted.'
    SourceMigrationHashRejected = 'Migration source checksums do not match the approved versions. No rebuild was attempted.'
    SecretsNotGitIgnored = 'Local encrypted configuration is not excluded by Git. Check repository ignore rules.'
    SecretsTrackedOrGitUnavailable = 'Git inspection failed or local secrets are tracked. Check repository configuration.'
    KeyStorageReadFailed = 'Cannot read existing encrypted keys. Run as the Windows user that created them.'
    CredentialStorageReadFailed = 'Cannot read existing encrypted credentials. Run as the Windows user that created them.'
}
try {
    ...
    if ($UpdateBackend) {
        ...
        Write-Host 'No process was stopped. Artifact recovery is attempted at most once, before service launch. Any opened service consoles remain under your control.'
    }
    exit 1
}
